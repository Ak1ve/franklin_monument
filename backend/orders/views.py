...
from .responses import OrderItemResponse, OrderOverviewResponse, UserTaskCommentResponse, OrderPlacementResponse
...
def order_new(request):
    ...
    proofs.save()
    names = models.PlacementNames()
    names.save()
    placement = models.MemorialPlacement(placement_names=names)
    placement.save()
...
def placement_edit(request, order_id: int):
    if request.method != "POST":
        return HttpResponseNotAllowed(b"{'err': 'Can only be POST'}")
    if not request.user.is_superuser:
        return HttpResponseServerError(b"{'err': 'Not authenticated'}")
    ...
    order: models.Order = models.Order.objects.get(id=order_id)
    response: OrderPlacementResponse = OrderPlacementResponse.from_json(request.body)
    order.placement.edit_from_response(response)
    return HttpResponse(b"{}")


def documents_upload(request, order_id: int):
    file = save_request_file(request.FILES["Upload"])
    order: models.Order = models.Order.objects.get(id=order_id)
    order.documents.files.add(file)
    order.documents.save()
    return JsonResponse({"file_name": file.short_name(), "file_id": file.id})


def documents_edit(request, order_id: int):
    order: models.Order = models.Order.objects.get(id=order_id)
    order.documents.notes = json.loads(request.body)["notes"]
    order.documents.save()
    return HttpResponse(b"{}")