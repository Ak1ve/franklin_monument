...
export function listCard<D>(
  schema: ModelSchema<D>,
  component: (props: ListCardProps<D>) => JSX.Element,
  fetchOnDock: boolean = true,
  navBase: boolean = true
): () => JSX.Element {
  return () => {
    const sessionStatus = useSession({ required: true }).status;
    const [data, setData] = useState(null as D);
    const [endpointError, setEndpointError] = useState(
      null as EndpointError | null
    );
      ...
      loadingMessage: (
        <>
          ...
          <StandardInfo
            ...
            hook={showInfoHook}
          >
            This page is currently loading. If this page loads for too long,
            ensure the system is working properly. It is possible the data is
            taking a while to load, or there is an error with how the data is
            setup.
            <br />
            If you can't currently access this page, you can navigate back to
            the dashboard.
          </StandardInfo>
        </>
      ),
      ...
      errorMessage: (
        <>
          ...
          >
            {isPermError ? (
              <>
                You are not allowed to view this page. If this is a mistake,
                contact an administrator. If you were recently granted
                permission to view this page, please wait 5-10 minutes before
                attempting to load again.
                <br />
                <br />
                Additional information: {endpointError.error}
              </>
            ) : (
              <>
                An unknown error has occured while loading this page. Please
                ensure that your account has been set up correctly.
                <br />
                <br />
                Additional informaiton: ${endpointError?.error}
              </>
            )}
          </StandardError>
        </>
