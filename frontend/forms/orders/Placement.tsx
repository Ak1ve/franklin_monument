...
function Bar() {
    ...
    return (
        <div ref={drag} className="cursor-move bg-black overflow-visible text-white w-[200%] select-none text-center align-middle relative">
            Monument
        </div>
    );
}

function PlacementBin({ children, onDrop, showBar }: { children: any, onDrop: () => any, showBar: boolean }) {
    let isHover = false;
...
function basicPlacement(id: string, hook: any) {
    return basicTextArea({id, label: "", inputClass: "!p-0 !mb-0 text-center", containerClass: "!p-0 !mb-0", placeholder: "Enter Text..."}, hook)
}
...
function PlacementArea() {
    ...
    const placementBins = Array(21).fill(null).map((_, index) => (
        <PlacementBin key={index} showBar={bins.bar === index} onDrop={() => setBins({ bar: index })}>
            <Bar />
        </PlacementBin>)
    );
