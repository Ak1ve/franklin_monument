...
function Order({ order }: { order: OrderType }) {
    const items = order.items.map(x => <span key={x} className="bg-gray-200 rounded-full px-2">{x}</span>)
    return (
        <div className="order-card-primary" key={order.id as any}>
            <div className="flex">
                <span className={classNames("mr-2 badge", colorStatus(order.status))}>{order.status}</span>
                <a className="hover:text-sky-300 underline text-lg font-medium" href={`/${order.id}`}>{order.deceased}</a>
            ...
            <div className="grid grid-cols-2 mx-3 my-2 text-gray-500 gap-3 text-sm w-full">
                <div className="divide-x-2 divide-purple-200">
                    ...
                    <div className="pl-2">Email: <a className="underline" href="#">{order.customer.email}</a></div>
                    <div className="pl-2">Webiste: <a className="underline" href="#">{order.customer.website}</a></div>
                </div>
                <div className="divide-x-2 divide-purple-200">
                    <div></div>
                    <div className="pl-2">Cemetery: {order.cemetery.name}</div>
                    <div className="pl-2">Address: <a className="underline" href="#">{order.cemetery.address}</a></div>
                </div>
            </div>
        ...
        <div className="gap-2 grid place-items-center my-4">
            <div className="mt-5 md:w-7/12 w-11/12 text-lg">
                ...
                <div className="relative">
                    ...
                    <button type="button" className="bg-sky-500 hover:bg-sky-600 text-gray-50 absolute right-2.5 bottom-2.5 focus:ring-4 focus:outline-none focus:ring-sky-300 font-medium rounded-lg text-sm px-4 py-2">Search</button>
                </div>
                <StretchButton onClick={() => setVisible(x => !x)} buttonClass="bg-sky-500 rounded-full text-gray-50 px-4">
                    Filter
                </StretchButton>
            </div>
